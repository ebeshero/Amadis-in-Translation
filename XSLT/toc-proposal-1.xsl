...
java -jar ../../../SaxonHE9-6-0-7J/saxon9he.jar -s:XML-and-Schematron/Southey XSLT/toc-proposal-1.xsl -o:html

    -->
    
    <xsl:variable name="montalvo" select="collection('../XML-and-Schematron/Montalvo')"/>
    <xsl:template match="/">
        <xsl:result-document href="{'../html/toc-1.html'}">
            <html>
                <head>
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="{($position + $southey-clauses) * $var + 155}" height="125">
                <g>
                    <rect y="12" fill="#d61d08" stroke="#550b03" stroke-width="2" height="20"
                        opacity="0.7" stroke-opacity="1" x="{$position * $var + 80}"
                ...
                <g>
                    <rect fill="#fdd221" stroke="#550b03" stroke-width="2" height="20"
                        opacity="0.7" stroke-opacity="1" width="{count($montalvo-clauses) * $var}" y="12" x="80"/>
