...
java -jar ../../../SaxonHE9-6-0-7J/saxon9he.jar -s:XML-and-Schematron/Southey XSLT/toc-proposal-4.xsl -o:html

    -->

<xsl:variable name="montalvo" select="collection('../XML-and-Schematron/Montalvo')"/>
<xsl:template match="/">
    <xsl:result-document href="{'../html/toc-4.html'}">
        <html>
            <head>
            ...
            <body>
                ...
                <ul>
                    ...
                    <li>Width depends on the length of that particular chapter.</li>
                </ul>
...
<xsl:template match="div[@type eq 'chapter']">
    ...
    <a href="{concat(replace($southey-chapter, '\s+', ''), '.html')}">
        <h2>
            ...
            <div class="svg">
                <svg xmlns="http://www.w3.org/2000/svg" width="{count($montalvo-clauses) + 5}" height="125">
                    <g>
                        <rect fill="#A04747" stroke="#220a00" stroke-width="2" height="20"
                            width="{count($montalvo-clauses) - count($montalvo-match)}" y="0" x="0"/>
                        <rect fill="none" stroke="#220a00" stroke-width="2" height="20" width="{count($montalvo-clauses)}"
                            y="0" x="0"/>
                        <text y="16" fill="#220a00" x="{count($montalvo-clauses) - count($montalvo-match) + 5}" font-size="13"><xsl:value-of
                            select="$omissions"/>%</text>
                    </g>
                    <text fill="#461801" y="65" x="2" font-size="18">Omissions by Southey</text>
                </svg>
                <svg xmlns="http://www.w3.org/2000/svg" width="{$southey-clauses + 5}" height="125">
                    <g>
                        <rect fill="#CDDB83" stroke="#220a00" stroke-width="2" height="20"
                            width="{count(current()//anchor[@type eq 'add'])}" y="0" x="0"/>
                        <rect fill="none" stroke="#220a00" stroke-width="2" height="20" width="{$southey-clauses}"
                            y="0" x="0"/>
                        <text y="16" fill="#220a00" x="{count(current()//anchor[@type eq 'add']) + 5}" font-size="13"><xsl:value-of
                            select="$additions"/>%</text>
                    </g>
                    <text fill="#461801" y="65" x="2" font-size="18">Additions by Southey</text>
                </svg>
            </div></a>
