...
java -jar ../../SaxonHE9-6-0-7J/saxon9he.jar -s:XML-and-Schematron/Southey XSLT/fs-to-html.xsl -o:html

    -->
    <xsl:template match="/">
        <xsl:result-document href="{'../html/charts.html'}">
            <html>
                <head>
                ...
                <body>
                    <h1>Alignment charts and statistcs</h1>
                    <hr/>
                    ...
                    <ul>
                        <xsl:for-each select="collection('../tables')//TEI">
                            ...
                            <li>
                                <a href="{concat('#', replace($chapter, '\s+', ''))}">
